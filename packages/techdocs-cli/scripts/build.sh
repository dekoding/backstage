...
TECHDOCS_CLI_EMBEDDED_APP_DIR="$TECHDOCS_CLI_DIR"/../techdocs-embedded-app
...
build_and_embed_app() {
  ...
  if [ "$TECHDOCS_CLI_DEV_MODE" = "true" ] ; then
    yarn workspace techdocs-embedded-app build:dev > /dev/null
  else
    yarn workspace techdocs-embedded-app build > /dev/null
  fi
